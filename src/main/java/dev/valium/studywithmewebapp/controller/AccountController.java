...
//    }
    ...
    @GetMapping("/check-email")
    public String checkEmailTokenForm(Model model) {
        model.addAttribute(new EmailVerificationForm());
        return "account/check-email";
    }

    @PostMapping("/check-email")
    public String confirmEmailToken(@CurrentUser Account account, Model model,
                                    @Valid EmailVerificationForm form, BindingResult result) {
        if(result.hasErrors()) {
            return "account/check-email";
        }

        Account fountAccount = accountRepository.findByEmail(account.getEmail());

        if(!form.getToken().equals(fountAccount.getEmailCheckToken())) {
            result.rejectValue("token", "invalid.token", new Object[]{form.getToken()}, "토큰이 올바르지 않습니다.");
            return "account/check-email";
        }

        accountService.completeVerification(fountAccount);

        model.addAttribute("numberOfUser", accountRepository.count());
        model.addAttribute("nickname", fountAccount.getNickname());
...
//    }
    ...
    public String viewProfile(@PathVariable String nickname, Model model, @CurrentUser Account account) {
        Account foundNickname = accountRepository.findByNickname(nickname);

        if(nickname == null) {
            throw new IllegalArgumentException(nickname + "에 해당하는 사용자가 없습니다.");
        }

        model.addAttribute(foundNickname); // 객체 타입의
